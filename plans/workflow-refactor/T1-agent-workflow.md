...
- WHY: 纯机械操作，不占认知资源，快速完成

## 理解（核心阶段——Agent 在此花最多思考力）
- 产出空间画像（引导维度：形状、动线、采光、关键资源）
- 确定空间类型 → 加载对应房间 Skill
- 评估分区需求 → 如需要，加载 generate-zoning Skill（T3 后可用）
- 空间类型 → Skill 映射表（简洁查找表形式，不需要 AI "理解"为什么这么映射）
- WHY: 理解空间本质后才能做出好决策，而非直接进入"放家具"模式

## 策略
- 遵循已加载的房间 Skill 产出策略声明
- 策略声明引导问题（主通道、关键墙面、家具清单）
- WHY: 策略是全局决策的记录，让执行时有明确方向，不陷入局部优化
...
- 修正循环（最多 2 轮）
- 兜底：移除违规家具，保留核心布局
- WHY: 信赖前面阶段的决策质量，执行阶段专注精确计算

## 审查
- 截图 → 基于设计原则整体评估
- 四个评估维度（动线、均衡、功能、品质）
- 发现问题 → 修正 → validate → 重新审查（最多 1 轮）
- WHY: 像设计评审一样审视方案，而非像质检员一样逐项检查
...
2. **新增：对话作为通用能力**（整合附录 A 调研成果）：
   ```
   设计理念（行为倾向层）：
   - 确认偏好只需几秒，但按错误方向完成整个布置的返工成本很高。
   - 好的设计师在战略选择点主动与客户沟通，在战术执行中独立决策。

   判断原则（泛化原则，非穷举）：
   - 如果存在两种方案，且选择不同会导致用户日常使用体验有质的差异
     （不是量的微调），则向用户展示推荐方案和替代方案。

   典型场景（非穷举，帮助 Agent 理解"质的差异"的含义）：
   - 功能区联动效应（移动大型家具释放子空间做独立功能区）
   - 动线策略重写（改变锚点家具的锚定墙导致整体分区重组）
   - 使用姿态选择（核心家具的墙面选择决定用户使用姿态和视线方向）
   - 空间性格选择（独立子空间的功能定位，如延伸区做更衣室或化妆间）

   行为规范：
   - 展示专业分析 → 推荐方案 + 替代方案 + 核心取舍（不是问问题）
   - 可以在理解后、策略前、或策略后触发（Agent 自主判断）
   - 用 "consider" 模式控制频率：明显能推断的不问，真的存在战略分歧才问

   禁止场景：
   - query / edit 任务
   - 战术级决策（间距、同墙面位置微调等由规范决定的问题）
   - 用户已明确设计偏好的情况
   ```
...
- [ ] 四个文件间**无信息重复**（职责单一原则）
- [ ] 每条规则都有 **WHY**（机制 3）
- [ ] 行数达标：workflow ≤150、bedroom ≤150、BIMCANVAS ≤100、layout-agent ≤80
- [ ] **validate_layout** 调用机制正确（执行阶段必调、layout-agent 传 zoneIds、修正循环最多 2 轮）
- [ ] 五阶段流程完整（感知→理解→策略→执行→审查→汇报）
- [ ] 房间 Skill 包含至少 **1 个策略声明示例**（机制 4：示例锚定基准）
- [ ] 对话能力：泛化原则 + 典型场景（非穷举） + 行为倾向描述（§2.2 要点 2）
- [ ] layout-agent 三约束明确：静默执行、单房间验证（zoneIds）、不派发任务
...
- [ ] 策略声明有引导结构
- [ ] 截图审查的评估维度是原则性的（非清单式）
- [ ] 修正循环有兜底策略（移除违规家具，保留核心布局）

**对话**：
- [ ] 对话定义为 Agent 通用能力（不是工作流固定阶段）
- [ ] 可以在工作流的任何阶段触发
- [ ] 禁止场景明确（query/edit、战术级、已明确偏好）
...
> **注意**：以下三个方向已整合到 §2.2 BIMCANVAS.md 和 §2.3 layout-agent.md 的调整要点中。此处保留原始调研推导过程作为参考。
