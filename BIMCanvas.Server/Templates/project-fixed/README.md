...
- generate 任务会先判定任务语义：
  - `derived` / `reference-informed-derived` → `generate-planning` → `generate-placement`
  - `reference-analysis` → `generate-reference-analysis` → `generate-planning` → `generate-placement`
