...
4. 收缩或删除附属件
5. 同类模块的小幅替换（不改变合同含义）
6. 执行 `v0.3` 明确写入的合同内 fallback（例如卧室睡眠组从 `1800床 + 双床头柜` 降为 `1500床 + 双床头柜`），前提是 fallback 不跨墙、不删除必需家具、不降低衣柜等级、不截断满墙窗帘

执行后统一记为 `[自动适配]`。

**WHY**：这些操作只是实现层面的微调，或执行设计师已经写入 `v0.3` 的预授权弹性，不改变设计意图。类比建筑施工中的"现场微调"——墙面内左右挪 50mm、旋转 5° 对齐墙面、缩小 100mm 避让管道，都不需要重新报审图纸。
...
例外：`v0.3` 已明确写入的合同内 fallback 不属于合同外语义改图，可按几何级修正自动执行并标记 `[自动适配]`。未写入 `v0.3` 的换墙、删家具、降衣柜、截断窗帘仍必须升级。
...
**合同冲突处理**：若 `validate_layout` 报错只能通过缩短满墙窗帘、降级床型、压缩衣柜等级等方式解决，说明 `v0.3` 合同与几何事实冲突。此时停止静默修正，按当前执行模式升级处理，不得为了得到 0 error 而改写设计意图。
