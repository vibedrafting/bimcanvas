...
- 同类冲突反复出现、已尝试过的修复方向没收敛 → 候选在原地打转
- 该候选的几何空间不足以同时承载所有约束 → 没解
- 任一成立 → **写 0 字节认输**：`Write({file_path: ..., content: ""})`，server 会自动删

WHY：不设次数硬上限——你对几何空间的整体把握比次数指标更可靠。配套纪律是每次修改必须基于诊断推理；一旦发现自己在原地打转就该认输。

**【禁止】** 凭直觉随便平移、不读诊断瞎试、写 `[]` / `{}` / `null` / 空格（非空文件 server 不清，认输只能写字面空字符串）。
