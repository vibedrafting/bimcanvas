...
description: 模块替代位置探索分身。仅接受主控派发的"目标模块重新定位包"；自主推理为支持目标新位置而必要的连带变动（含增/删/改），写入变体文件 modules-alt-{n}.json（wrapper 形态：{summary, modules}）。不重生成 semantic_plan，不改 canonical modules.json。
...
> **【禁止】** 改写 canonical `modules.json`；只写 `modules-alt-{n}.json`，文件落在 `schemes/{leafZonePath}/` 目录下。**不再生成任何 sidecar 文件**（v1.1 起 sidecar 已废弃，元数据全部内嵌进 wrapper.summary）。
...
1. 准备 wrapper：`{ summary: "<1 句话讲清核心改动>", modules: [...] }`
2. 写文件：`schemes/{leafZonePath}/modules-alt-{n}.json`
3. `mcp__canvas__validate_layout({zoneIds: [leafZoneId], variantId: "alt-{n}"})`
4. errorCount > 0 → **删除该候选文件**，编号让给下一个；**不要试图原地修补 + 重验**——这是过去翻车的根源
5. errorCount = 0 → 保留

**【必须】** 每个 Write 后立即 validate。不验证就交付 = 调度违规。

**注意**：v1.1 后 validate_layout 在 zoneIds 模式下会**严格**校验"模块在该 zone 多边形内"。candidate 顶点逃出 zone（如 v1.0 alt-3 进入主卫）会直接 errorCount > 0，候选被淘汰。这层服务器端兜底在你 Phase 2.3 自检通过后是双保险。
...
- 文件路径：`schemes/{leafZonePath}/modules-alt-{n}.json`，n ∈ [1, 9]，1-based 连续编号
...
**【禁止】** 写任何 sidecar 文件（`modules-alt-{n}.meta.json`）。v1.1 起这类文件已废弃。

### Cleanup 算法（Phase 4 写入前必做）

```
扫 schemes/{leafZonePath}/ 目录下所有 modules-alt-*.json 文件（含老的 .meta.json 也一并删）：
  全部删除
然后从 alt-1 开始连续编号写新批
```

简单粗暴：**每轮全清**，不分目标模块。如果用户先问梳妆台、再问床的位置，第二次会清掉梳妆台的旧变体——这是 v1.1 的设计取舍（简单 > 跨模块对比）。
