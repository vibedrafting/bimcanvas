...
```json
{
  "success": true,
  "data": {
    "text": "客厅区域（8m × 6m）共有 3 件家具：\n\n1. 三人沙发（f_001）：位于房间南侧中央（3000, 4500），面向北墙，属于客厅主区。\n2. 圆形茶几（f_002）：位于沙发正前方 650mm 处（3000, 3500），便于使用。\n3. 阅读单椅（f_003）：位于东北角阅读区（7000, 1500），斜向摆放利用自然光。\n\n空间关系：沙发面向窗户方向，茶几与沙发保持合理距离，阅读椅靠近窗户。",
    "summary": {
      "totalElements": 3,
      "byZone": {
        "zone_living": 2,
...
| `position` | Point2D | 是 | 中心点位置 `{ x, y }` (mm) |
...
| `position` | Point2D | 是 | 新的中心点位置 `{ x, y }` (mm) |
...
```json
{
  ...
  "data": {
    "elementId": "f_001",
    "previousPosition": { "x": 3000, "y": 4500 },
    "newPosition": { "x": 3500, "y": 4500 },
    "newGridPosition": { "row": 9, "col": 7 },
    "zoneChanged": false,
    "collisions": [],
    "updatedRelations": [
...
| `angle` | number | 是 | 旋转角度（度），支持绝对值或相对值 |
...
|------|------|------|------|
| `canvasId` | string | 是 | 画布 ID |
| `position` | Point2D | 是 | 查询位置 `{ x, y }` (mm) |
...
```json
{
  ...
  "data": {
    ...
    "nearbyStructure": [
      { "type": "wall", "id": "wall_001", "distance": 3500 }
    ]
  },
  "version": 19,
  "pendingCommits": []
