...
- **TodoWrite 进度控件**: `TodoWrite` 工具调用会在聊天滚动区底部悬浮显示 Codex 风格临时进度面板，位于上下文状态栏和输入框上方，右侧避开滚动条区域；聊天滚动容器会按控件真实高度上移底边，避免遮挡对话气泡。控件支持折叠；未完成任务会跨普通新对话保留，直到后续 `TodoWrite` 更新为全部完成，或本轮失败 / 中止后自动关闭。
