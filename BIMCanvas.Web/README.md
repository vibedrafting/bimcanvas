...
- **TodoWrite 进度控件**: `TodoWrite` 工具调用会在聊天滚动区底部悬浮显示 Codex 风格临时进度面板，位于上下文状态栏和输入框上方，右侧避开滚动条区域；后台等待提示显示在 Todo 面板下方，聊天滚动容器会按底部状态组真实高度上移底边，避免遮挡对话气泡。控件支持折叠；未完成任务会跨普通新对话和页面刷新保留，直到后续 `TodoWrite` 更新为全部完成，或本轮失败 / 中止后自动关闭。
...
- **截图附件资源化**:
    ...
    - 失败或中止时恢复附件草稿，避免重新截图
- **等待期间提前发送**:
    - Agent 流式回复期间，输入框仍可填写后续请求；有内容时右侧按钮从停止切换为发送，并将内容暂存为单条等待发送消息。
    - 等待消息会捕获当时的文本、附件和 Space Mark 快照；当前轮正常完成后自动发送，失败或中止时保留。
    - 等待条提供立即发送、撤回编辑、删除三个图标操作；等待消息仅存在于当前页面生命周期内，不做刷新恢复。
- **后台截图渲染**:
...
- **运行时能力降级**: `/api/config.capabilityMatrix` 会驱动 Thinking 开关、Tasks 视图等前端降级；仅当能力等级为 `unsupported` 时才启用 `frontendFallback`。`optional` 表示运行时支持该能力但事件可能不会每次产生，例如 thinking 气泡只在模型实际输出 `thinking.delta/thinking.completed` 时显示。
...
- **StandaloneRuntime**：`saveModules()` 只确认内存编辑，不写本地文件系统。用户需要通过 Export 导出 Snapshot JSON；关闭或刷新页面前未导出的内存修改不会自动持久化。
...
| ✅ 导出 | Standalone 下通过 Snapshot JSON 交付编辑结果 |
