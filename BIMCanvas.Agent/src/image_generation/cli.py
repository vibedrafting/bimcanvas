...
9. 输出中的家具模块数量、家具标签数量和各类标签的重复次数，都不得超过第一张图中可明确识别的内容；严禁为了补箭头、补构图或让画面更完整而新增家具、复制家具、补写额外标签。
10. 任何家具标签都必须直接来自第一张图中已经清晰可见的原始中文标签；不允许改名、不允许补全、不允许把模糊文本猜成新标签。如果标签不清楚，就省略该家具和该标签。
11. 方向箭头不是必须项，只是低优先级附加标记；只有在某个大型家具模块已经被准确保留、标签明确且朝向非常明确时，才可以补一个小型、细线、简洁的暖红/粉红方向箭头。
12. 箭头绝不能触发新增家具、重复家具、额外标签或额外结构；如果“加箭头”与“避免幻觉”发生冲突，永远选择不画箭头。
13. 箭头优先只用于床、长条柜体、衣柜排布这类大件家具；床头柜、梳妆台、小柜体、卫浴器具默认不画箭头，除非第一张图里其方向极其明确且不会引发误判。
14. 每个被允许画箭头的家具最多只画一个箭头，箭头要小，不要遮挡家具标签和家具外轮廓，优先放在家具内部空白处或紧邻家具边缘。
15. 只给家具画方向箭头，不给墙、门、窗画箭头；如果方向不明确，宁可不画，也不要猜测或额外创造朝向信息。
16. 门和窗都要尽量识别并保留其图形位置与形态，但不要输出任何门标签或窗标签，不要在图上写“门”“窗”。
17. 对于边界模糊、细节不清的门窗，不要强行重绘成规则新图形，优先保留原图中已有的开口轮廓和细节，只做轻度风格统一，避免幻觉。
18. 墙体、门、窗、柜体、家具请尽量使用纯色块面和清晰轮廓，不要保留墙体内部填充图案、剖面斜线、CAD 杂线、尺寸辅助线、隐藏线或施工纹样。
19. 输出必须是清晰的 2D 正投影平面图，不要透视、不要 3D、不要照片感、不要真实材质、不要光影、不要体积渲染。
20. 如果第一张图与第二张图冲突，永远服从第一张图的结构与布局，只借用第二张图的配色与图形语言。"""
