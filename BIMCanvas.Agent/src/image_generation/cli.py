...
9. 门和窗都要尽量识别并保留其图形位置与形态，但不要输出任何门标签或窗标签，不要在图上写“门”“窗”。
10. 对于边界模糊、细节不清的门窗，不要强行重绘成规则新图形，优先保留原图中已有的开口轮廓和细节，只做轻度风格统一，避免幻觉。
11. 墙体、门、窗、柜体、家具请尽量使用纯色块面和清晰轮廓，不要保留墙体内部填充图案、剖面斜线、CAD 杂线、尺寸辅助线、隐藏线或施工纹样。
12. 输出必须是清晰的 2D 正投影平面图，不要透视、不要 3D、不要照片感、不要真实材质、不要光影、不要体积渲染。
13. 如果第一张图与第二张图冲突，永远服从第一张图的结构与布局，只借用第二张图的配色与图形语言。"""
