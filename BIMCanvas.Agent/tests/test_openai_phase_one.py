...
def test_openai_capability_matrix_marks_subtask_as_unsupported_after_v0_1_收口() -> None:
    subtask_row = next(
        row
...
def test_openai_settings_default_to_chat_completions_for_custom_base_url(
    ...
    # BIMCanvas OpenAI Runtime v0.1 收口：默认主路径是 chat_completions + streaming，
    # 即使在第三方 endpoint 下也一样（这是"对第三方 provider 友好"的核心承诺）。
    home = _prepare_bimcanvas_home(tmp_path)
    _configure_test_home(monkeypatch, home)
    _set_openai_runtime_config(
        home,
    ...
    assert settings.openai_api == "chat_completions"
    assert settings.openai_disable_tracing is True


def test_openai_settings_reject_responses_with_third_party_endpoint(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    # v0.1 收口：第三方 OpenAI-compatible endpoint + responses 组合不再静默降级为
    # 非流式缓冲 fallback；必须在 Settings.load() 阶段诚实拒绝。
    home = _prepare_bimcanvas_home(tmp_path)
    _configure_test_home(monkeypatch, home)
    _set_openai_runtime_config(
        home,
...
def test_openai_agent_responses_run_fallback_is_disabled_after_v0_1_收口(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    # _should_use_responses_run_fallback 保留签名供历史测试平滑过渡，
    # 但语义固定为 False（主路径永远走 run_streamed）。
    home = _prepare_bimcanvas_home(tmp_path)
    _configure_test_home(monkeypatch, home)
    _set_openai_runtime_config(
        home,
    ...
    agent = OpenAIAgent(project_path=str(tmp_path), working_directory=str(tmp_path))
    assert agent._should_use_responses_run_fallback(get_settings()) is False
...
def test_openai_agent_reuses_sdk_session_for_same_host_session(
    ...
    # v0.1 收口：OpenAI Runtime 主路径为 chat_completions + run_streamed。
    # 本测试验证同一 host sessionId 下两次 chat_stream 复用同一个 SDK session。
    home = _prepare_bimcanvas_home(tmp_path)
    _configure_test_home(monkeypatch, home)
    _set_openai_runtime_config(
        home,
    ...
    fake_agents.Runner = SimpleNamespace(run_streamed=fake_run_streamed)
...
def test_openai_stream_translator_backfills_missing_tool_completions_from_agent_tool_payload() -> None:
    translator = OpenAIStreamTranslator(turn_id="turn-1")
    _, subtask_id = translator.ensure_subtask_started_for_tool_call(
        SimpleNamespace(
            call_id="layout-call-1",
    ...
    payload = {
        "summary": "已完成单区 generate 并通过校验。",
        "completedToolCalls": [
            {"providerCallId": "call-2", "toolName": "Write", "output": "Wrote modules"},
            {"providerCallId": "call-3", "toolName": "mcp__canvas__validate_layout", "output": "validated"},
        ],
    }
    completion_chunks = translator.translate_result_item(
        SimpleNamespace(
            type="tool_call_output_item",
            raw_item={"call_id": "layout-call-1", "type": "function_call_output"},
            output=f"{AGENT_TOOL_RESULT_MARKER}{json.dumps(payload, ensure_ascii=False)}",
            tool_origin=SimpleNamespace(type="agent_as_tool"),
        )
    )

    assert [
        (chunk.type, chunk.subagent_id, chunk.tool_name, chunk.tool_output, chunk.success)
        for chunk in completion_chunks[:-1]
    ] == [
        ("tool_call_complete", subtask_id, "Write", "Wrote modules", True),
        ("tool_call_complete", subtask_id, "mcp__canvas__validate_layout", "validated", True),
    ]
    assert [(chunk.type, chunk.subagent_id, chunk.content, chunk.success, chunk.error) for chunk in completion_chunks[-1:]] == [
        ("subagent_complete", subtask_id, "已完成单区 generate 并通过校验。", True, None)
    ]
...
def test_openai_agent_custom_output_extractor_emits_structured_agent_tool_payload() -> None:
    result = SimpleNamespace(
        final_output=None,
        new_items=[
            SimpleNamespace(
                ...
                raw_item=SimpleNamespace(
                    call_id="call-2",
                    name="Write",
                    arguments='{"file_path":"schemes/rz_1/modules.json","content":"[]"}',
                ),
            ),
            SimpleNamespace(
                type="tool_call_output_item",
            ...
            SimpleNamespace(
                ...
                raw_item=SimpleNamespace(
                    call_id="call-3",
                    name="mcp__canvas__validate_layout",
                    arguments='{"zoneId":"rz_1"}',
                ),
            ),
            SimpleNamespace(
                type="tool_call_output_item",
    ...
    output = asyncio.run(OpenAIAgent._extract_agent_tool_output(result))

    assert output.startswith(AGENT_TOOL_RESULT_MARKER)
    payload = json.loads(output[len(AGENT_TOOL_RESULT_MARKER):])
    assert payload["summary"] == "已完成单区 generate 并通过校验。"
    assert payload["completedToolCalls"] == [
        {"providerCallId": "call-2", "toolName": "Write", "output": "Wrote modules"},
        {"providerCallId": "call-3", "toolName": "mcp__canvas__validate_layout", "output": "validated"},
    ]
