...
- 异形空间：rawBoundary 顶点 > 4，且延伸/凹角部分面积 ≥ 3m²（单件家具占用的窄段如淋浴尽端 ≈ 1m² 不算独立子空间）
- 多功能组合：多个功能标签 + 面积足以分区
...
> ⚠ 反模式：信号命中后跳过 Skill 加载。常见误判：用面积直觉（"延伸区太小"）或功能预判（"只能做淋浴区"）替代 Skill 的预布置验证。延伸部分 ≥ 3m² 时【必须】加载 Skill；< 3m² 时可直接进入策略（作为功能区的一部分处理）。
