...
**衣柜是卧室的主要家具**：阶段 A 应与床一同规划。次卧中，仅当所有可用墙面放置 600mm 深柜体后主通道 < 600mm 时才允许省略。
...
卧室衣柜的墙面选择直接决定动线品质。在规划衣柜位置前，必须评估所有候选墙面的**有效连续段长度**：

1. **列出候选墙面**：排除窗墙和床头墙，剩余墙面均为候选
2. **计算每面墙的有效段**：
   - 无门窗墙面：有效段 = 整面墙长度
   - 有门/窗的墙面：有效段 = 扣除门窗开启区域（门前 900mm 净空）后的**最长连续剩余段**
   - 注意：有门的墙 ≠ 不可用的墙。例如 5000mm 墙面有一扇 900mm 门，门后剩余段可能达 4000mm+
3. **选择最优墙面**：取有效段最长的墙面放置衣柜
4. **记录决策过程**：在 placementReason 中写明各候选墙面的有效段长度和选择理由

> 常见误判 1：Agent 容易把"有门的墙"整面排除。必须计算门两侧的剩余段长度后再做比较。
> 常见误判 2：L 形或异形房间中，Agent 容易只评估床所在子区域的墙段。衣柜不必与床在同一子区域——必须穷举房间边界上的所有墙段后再比较（参见 placement_guide §5.2）。

### 6A. 放置前预检

对每件要放置的家具，在确定坐标前检查：
1. 是否阻挡门开启（门前 900mm 净空）？
2. 通行间隙是否满足通道标准：主通道>=900mm、次通道>=600mm、床使用侧>=500mm？
   注意：先区分「通行间隙」与「使用间隙」（参见 placement_guide §4.1），使用间隙（如床头柜与相邻衣柜之间）不适用通道标准。
   常见误判：「床使用侧到对面衣柜」属于床侧通道（≥500mm），不是主通道（≥900mm）。
   间距分配（主卧、次卧均适用）：先为两侧床头柜各预留 ≥ 其深度的空间（窗侧在窗帘之外），剩余深度偏向使用侧。仅紧凑房型（两侧放下床头柜后使用侧 < 500mm）窗侧取最小值 200mm（参见 placement_guide §5.2 + §5.3）。
3. **衣柜墙面选择**：是否已完成 §5.6 的有效段评估？是否选择了有效段最长的墙面？
...
- [ ] H5: 通行间隙满足通道标准（主>=900、次>=600、床使用侧>=500）；使用间隙（如床头柜旁）不计入

**设计规则（优先修正）**：
- [ ] S1: 锚点家具沿床头墙定位合理（参照 §5.2/§5.3 的布局间距分配规则）
- [ ] S2: 家具朝向符合 topology_rules / relation_rules
- [ ] S3: 衣柜沿最长连续有效段放置，且应填满有效段全长（标准柜体阵列 + 定制柜体补充）；有效段 = 扣除门窗开启区域后的连续可用长度；有门 ≠ 不可用
- [ ] S4: 床头靠实墙（非窗墙、非门墙）；次卧/主卧优先「侧对窗户」布局（床长边朝窗）；床脚不正对门
- [ ] S5: 成套家具完整（床+床头柜≥1、书桌+椅子）；卧室缺少床头柜须说明省略原因（净空<500mm）；有窗户的卧室/客厅应有窗帘
